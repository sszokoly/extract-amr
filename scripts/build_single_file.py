...
def emit_readme(output_dir: Path, shebang: Optional[str]) -> None:
    ...
    readme = (
        ...
        "\n"
        "## Inspect a packet capture to see AMR or AMR-WB reports\n"
        "\n"
        "```console\n"
        f"{python} extract-amr-asbce.py inspect call.pcapng --progress\n"
        "```\n"
        "\n"
        "## Extract an AMR flow from a packet capture as reported by inspect\n"
        "\n"
        "```console\n"
        ...
        "```\n"
        "\n"
        "## Extract an AMR-WB flow from a packet capture as reported by inspect\n"
        "\n"
        "```console\n"
        f"{python} extract-amr-asbce.py extract call.pcapng --progress \\\n"
