...
def build(output_dir: Path, keep_flat: bool, shebang: Optional[str]) -> Path:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        rebuild = build(arguments.output_dir / ".determinism", False, arguments.shebang)
        if digest(bundle_path) != digest(rebuild):
            raise BuildError("build is not deterministic")
        rebuild.unlink()
        rebuild.parent.rmdir()
